...
<main>
    <div class="container-xl p-10">
        ...
        <p style="color:#FF0000;font-weight:bold;margin:10px 0;"><?= $cart_obj->getErrors(); ?></p>
        

        <?= $productView->ProductAsLists($product_obj->getProducts()); ?>       

        <?php
        if ($cart_obj->getNumberOfItems() > 0) { 
          echo $cartView->cartItemsAsLists($cart_obj->getCartItems());
          ?>        
          <p>Total value of this cart: <b><?= formatValue($cart_obj->showTotalValue()); ?></b></p>
          <p>Total value VAT: <b><?= formatValue($cart_obj->createTotalVAT()); ?></b></p>
          <?php 
        } 
